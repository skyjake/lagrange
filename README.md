...
2. Check that you have the recommended build tools and dependencies installed: SDL 2, OpenSSL 1.1.1, libpcre, libunistring, GNU FriBidi, and zlib. For example, on macOS this would do the trick (using Homebrew): `brew install cmake automake sdl2 openssl@1.1 pcre libunistring fribidi` Or on Ubuntu: `sudo apt install cmake zip libsdl2-dev libssl-dev libpcre3-dev zlib1g-dev libunistring-dev libfribidi-dev`
...
5. In your empty build directory, run CMake to configure: `cmake .. -DCMAKE_BUILD_TYPE=Release`
6. Build it: `cmake --build .`
7. Now you can run _lagrange_, _lagrange.exe_, or _Lagrange.app_.
...
Adding messages to PO files requires `po/compile.py` to be run to create the binary representation, which then has to be included in Lagrange's resource zip by [configuring the project again](#how-to-compile).
